...
export const exportCanvasToPptx = async (
  ...
  titleSlide.addText(templateName, {
    x: 0.5,
    y: 3.5,
    w: 12.33,
    h: 0.8,
  ...
  for (const group of sectionGroups) {
    ...
    group.forEach((sectionId, colIndex) => {
      ...
      if (items.length === 0) {
        ...
        items.forEach((item, itemIndex) => {
          const yPos = 1.0 + itemIndex * 0.8;
          if (yPos > 6.5) return; // Don't overflow the slide
    ...
    if (resolvedWorkspaceName) {
      slide.addText(resolvedWorkspaceName, {
        x: 0.5,
        y: 7.08,
        w: 4.2,
        h: 0.2,
